...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndPrefabModelView.EnableDocking(CBRS_ALIGN_LEFT);
	DockPane(&m_wndMeshView);
	CDockablePane* pTabbedBar = NULL;
	//m_wndClassView.AttachToTabWnd(&m_wndFileView, DM_SHOW, TRUE, &pTabbedBar);
	//m_wndMeshView.AttachToTabWnd(&m_wndFileView, DM_SHOW, TRUE, &pTabbedBar);
	m_wndPrefabModelView.AttachToTabWnd(&m_wndMeshView, DM_SHOW, TRUE, &pTabbedBar);
	...
	m_pRenderViewDlg = std::make_shared<HrRenderViewDlg>(&m_wndPrefabModelView);
	...
	m_wndPrefabModelView.SetMainFrame(this);
	//RECT rc;
...
BOOL CMainFrame::CreateDockingWindows()
{
	...
	CString strPrefabModelView;
	strPrefabModelView = L"PrefabModelView";
	if (!m_wndPrefabModelView.Create(strPrefabModelView, this, CRect(0, 0, 200, 200), TRUE, ID_VIEW_CLASSVIEW, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI))
	{
		TRACE0("Failed to create Class View window\n");
		return FALSE; // failed to create
	}
...
void CMainFrame::SetDockingWindowIcons(BOOL bHiColorIcons)
{
	...
	HICON hPrefabModelViewIcon = (HICON) ::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(bHiColorIcons ? IDI_CLASS_VIEW_HC : IDI_CLASS_VIEW), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
	m_wndPrefabModelView.SetIcon(hMeshViewIcon, FALSE);
...
void CMainFrame::OnButtonSavePrefabText()
{
	...
	TCHAR szFilters[] = _T("MyType Files (*.hrpref)|*.hrpref|All Files (*.*)|*.*||");

	// Create an Open dialog; the default file name extension is ".my".
	CFileDialog fileDlg(FALSE, _T("hrpref"), L"",
...
void CMainFrame::OnBtnHrOpenPrefab()
{
	TCHAR szFilters[] = _T("hrpref Files (*.hrpref)|*.hrpref||");

	// Create an Open dialog; the default file name extension is ".my".

	CFileDialog fileDlg(TRUE, _T("hrpref"), _T(".hrpref"),
		OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, szFilters);
	...
	if (fileDlg.DoModal() == IDOK)
	{
		...
		std::string strFile = Hr::HrStringUtil::WstringToUtf8(pathName.GetBuffer());
		m_strCurrentOpenFile = Hr::HrFileUtils::Instance()->GetFileName(strFile);
		m_pRenderViewDlg->OnOpenPrefab(strFile);
	}
}
