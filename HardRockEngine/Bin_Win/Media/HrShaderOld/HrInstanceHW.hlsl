...
cbuffer cbPerFrame
{
	...
	float4x4 world_matrix_array[80];
...
VertexOut Pass0_VS_Main(VertexIn vIn)
{
    ...
    float4x4 worldViewProjMatrix = mul(world_matrix_array[vIn.insIndex], view_proj_matrix);
    float4x4 transWorldMatrix = transpose(world_matrix_array[vIn.insIndex]);
    vOut.posWorldViewProj = mul(float4(vIn.vertexPosition, 1.f), worldViewProjMatrix);
    vOut.posWorld = mul(float4(vIn.vertexPosition, 1.0f), world_matrix_array[vIn.insIndex]);

	//·¨Ïß±ä»»
    vOut.pixWorldNormal = normalize(mul(float4(vIn.vertexNormal, 0.0f), transWorldMatrix).xyz);
    vOut.uv = vIn.uv;
